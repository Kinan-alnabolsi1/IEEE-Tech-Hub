...
import { Plus, Clock, CheckCircle2, AlertCircle, Trash2, Edit2, Calendar, User, Loader2 } from 'lucide-react';
...
const TasksBoard = () => {
        ...
        try {
            ...
            if (!pId || pId === 'undefined' || pId === 'null') {
                ...
                const leaderProject = userData?.projects?.find(p => 
                    p.role_in_project?.toLowerCase().includes('leader')
                );
                if (leaderProject) {
            ...
            setTasks(tasksList);
            console.log("🔥 Tasks Loaded:", tasksList);
            ...
            const approvedVolunteers = members.filter(m => {
                ...
                return (stat.toLowerCase() === 'approved' || stat.toLowerCase() === 'active') && 
    ...
    const openModal = (task = null) => {
        if (task) {
            setTaskToEdit(task);
            const assignedIds = task.assigned_users?.map(u => u.user_id || u.id) || [];
            setFormData({
                title: task.title,
...
{columns.map(col => {
    // 🌟 الفلترة "العبقرية": تتجاهل الفراغات، الأحرف الكبيرة، وتطابق المسميات
    const colTasks = tasks.filter(t => {
        const taskStatus = (t.status || '').toLowerCase().trim();
        const columnId = col.id.toLowerCase().trim();
        
        // إذا كان العمود "To Do" (Pending)
        if (columnId === 'pending') {
            return taskStatus === 'pending' || taskStatus === 'to do' || taskStatus === 'todo';
        }
        
        // الحالات الأخرى (In Progress, Completed)
        ...
        <div key={col.id} className="flex-1 min-w-[320px] max-w-[400px] flex flex-col bg-slate-50/50 rounded-[2.5rem] border border-slate-100 p-2">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 px-2 custom-scrollbar">
                {colTasks.length > 0 ? colTasks.map(task => (
                    <div key={task.task_id || task.id} className="bg-white p-5 rounded-[1.8rem] shadow-sm border border-slate-100 group hover:shadow-md transition-all animate-in fade-in slide-in-from-bottom-2">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                            {teamMembers.length > 0 ? teamMembers.map(m => {
                                ...
                                <p className="col-span-2 text-[10px] text-amber-500 italic p-2 bg-amber-50 rounded-lg border border-amber-100 text-center">No active members found to assign.</p>
                            )}
                        </div>
                    </div>
